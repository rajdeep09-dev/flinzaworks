...
/* The roles live in data, not inline here — see the note where the array used to be. */
...
/* The role list used to be declared right here, inline.
 *
 * It is in `@/data/roles` now for two reasons. The first is technical: the server component that
 * renders this route emits JobPosting structured data for every open role — the block that makes a
 * role eligible for Google Jobs — and a server component cannot read an array declared inside a
 * `'use client'` file. The second is that the inline list still advertised an "AI UGC Producer",
 * for a service the studio has stopped selling, and a job ad for work nobody does is worse than a
 * blank page. Both problems are solved by there being one list, in data, that the page and the
 * schema both read. */
  ...
  return (
    ...
    >
      ...
      <PageToc
        label="Careers"
        items={[
          { id: 'careers-intro', label: 'The studio' },
          { id: 'careers-talent', label: 'Join the pool' },
      ...
      <section id="careers-intro" style={{ maxWidth: 900, margin: '0 auto', padding: '120px 24px 56px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 'clamp(15px, 1.7vw, 19px)', lineHeight: 1.6, color: '#52525b', margin: '0 auto', maxWidth: 620 }}>
          We&apos;re a remote-first ecommerce growth agency. Fast testers, honest readers of data, allergic to vanity metrics. If that sounds like you — pick a role, or join the pool below.
        </p>
      </section>

      {/* ── The talent pool ──
          The Atomik-shaped capture card the client asked for: dark, centred, and asking for all six
          things in one place. It sits ABOVE the role list on purpose. Somebody who does not match a
          listed role is still somebody worth having a name for, and previously the only way in was a
          text link at the very bottom of the page. */}
      <TalentPoolCard roles={roles} />
